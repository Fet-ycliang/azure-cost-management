...
- **Azure 平台 catalog 實際情況**：Genie 可能根據查詢範疇自動選用 `system_catalog` 或 `rag_analyst_catalog` 作為 Azure 平台資料來源。整體費用查詢傾向用 `system_catalog`；針對特定專案（如 CDP）可能改用 `rag_analyst_catalog`。**以 Genie 實際生成的 SQL 為準**，不要假設只有 `system_catalog`。
